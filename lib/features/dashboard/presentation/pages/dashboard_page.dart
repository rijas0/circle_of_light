...
class _DashboardPageState extends State<DashboardPage> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _buildCheckTile(isRead: true)),
              const SizedBox(width: 12),
              Expanded(child: _buildCheckTile(isRead: false)),
            ],
          ),
  ...
  Widget _buildCheckTile({required bool isRead}) {
    final bg = isRead ? const Color(0xFF3D7A52) : const Color(0xFFD4A96A);
    ...
    final sublabel = isRead ? 'Alhamdulillah!' : 'JazakAllah Khair!';
    final sublabelColor = isRead ? const Color(0xFF86EFAC) : _amber;

    return GestureDetector(
      onTap: () => setState(
          () => isRead ? _readToday = !_readToday : _reflectedToday = !_reflectedToday),
      child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: bg,
  ...
  Widget _memberAvatar({String? emoji, bool done = false, bool isUnknown = false}) {
    return Container(
      width: 46,
      height: 46,
      decoration: BoxDecoration(
  ...
  Widget _buildNavBar() {
    final items = [
      ...
      _NavItem(Icons.chat_bubble_rounded, Icons.chat_bubble_outline, 'Reflections'),
